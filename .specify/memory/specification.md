...
針對「手機放包包」的運動場景（如籃球 3v3 鬥牛），提供：
- 運動中：穩定的背景心率記錄（不需要看螢幕或聽語音）
- 運動後：詳細的數據分析和長期趨勢追蹤

**使用情境**：
- 戴心率帶，手機放包包
- 打 1.5-2.5 小時 3v3 鬥牛
- 有多回合上場/下場（先得 6 分的隊伍留下）
- 運動後查看報告和趨勢
...
**偵測邏輯**：
```
HR > 120 持續 30 秒 → 判定「上場」開始
HR < 100 持續 60 秒 → 判定「下場」休息
```

**應用價值**：
| 情境 | 記錄到的資料 |
|------|-------------|
| 連贏 3 場 | 連續高強度 25 分鐘，心率持續 160+ |
| 輸了等很久 | 休息 15 分鐘，完整恢復曲線 |
| 隊伍少輪很快 | 休息短，平均心率較高 |
...
{
  id: "uuid",
  sport: "basketball", // basketball | cycling | swimming | hiking | other
  startTime: "2025-12-06T18:00:00",
  endTime: "2025-12-06T19:30:00",
  duration: 5400, // 秒

  // 原始心率取樣（每秒一筆）
  samples: [
    { timestamp: 1733500800000, hr: 85 },
    { timestamp: 1733500801000, hr: 87 },
    // ...
  ],
  ...
  rounds: [
    {
      type: "active",      // active | rest
      startTime: 1733500800000,
      endTime: 1733501280000,
      duration: 480,       // 8 分鐘
      avgHR: 156,
      maxHR: 172
    },
    {
      type: "rest",
      startTime: 1733501280000,
      endTime: 1733501460000,
      duration: 180,       // 3 分鐘
      avgHR: 98,
      recoveryRate: 25     // 1 分鐘降 25 BPM
    },
    // ...
  ],
  ...
  summary: {
    ...
    // 回合統計
    totalRounds: 8,
    activeTime: 4500,      // 上場總時間（秒）
    restTime: 900,         // 休息總時間（秒）
    longestActive: 1500,   // 最長連續上場（秒）= 連贏場次
    avgRoundDuration: 562, // 平均每回合（秒）

    // 恢復力指標
    avgRecoveryRate: 28,   // 平均 1 分鐘下降 BPM
...
| 項目 | 方案 |
|------|------|
| 儲存 | IndexedDB（大量 samples） |
| 背景運行 | Web Worker + Screen Wake Lock API |
| 圖表 | Chart.js 或 ECharts |
| 離線 | PWA + Service Worker |
| 自動切分 | 心率狀態機（threshold + debounce） |
...
// 狀態：'idle' | 'maybe_active' | 'active' | 'maybe_rest' | 'rest'
```

### 11.7 開發階段

**Phase 1 (MVP)**：
- [ ] 運動類型選擇（籃球/騎車/游泳/健行/其他）
- [ ] 背景心率記錄（每秒取樣）
- [ ] 自動回合切分
- [ ] 單次報告（基本統計 + 回合列表）
- [ ] 歷史列表
...
| 現有功能 | 運動日誌模式 |
|---------|-------------|
| 打怪模式 | 適合室內訓練、可以看螢幕的場景 |
| 運動日誌 | 適合戶外運動、手機放包包的場景 |
| 心率監測 | 共用底層藍牙連接和心率數據 |

**整合方式**：
- 主頁新增「運動日誌」入口
- 共用 `heartRate` store 的藍牙連接
- 新增 `sportSession` store 管理運動記錄
- 新增 `SportSessionPage.vue` 和 `SessionReport.vue` 組件
